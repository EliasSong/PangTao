...
Socket::ptr Socket::CreateTCP(PangTao::Address::ptr address) {
    Socket::ptr sock(new Socket(address->getFamily(), TCP, 0));
    return sock;
}

Socket::ptr Socket::CreateUDP(PangTao::Address::ptr address) {
...
int Socket::sendTo(const iovec* buffers, size_t length, const Address::ptr to, int flags) {
    if(isConnected()) {
        ...
        msg.msg_namelen = to->getAddrLength();
        return ::sendmsg(m_sock, &msg, flags);
    }
    return -1;
}
...
int Socket::recvFrom(iovec* buffers, size_t length, Address::ptr from, int flags) {
    if(isConnected()) {
        ...
        msg.msg_namelen = from->getAddrLength();
        return ::recvmsg(m_sock, &msg, flags);
    }
    return -1;
}
...
bool Socket::cancelWrite() {
    return IOManager::GetThis()->cancelEvent(m_sock, PangTao::IOManager::WRITE);
}
